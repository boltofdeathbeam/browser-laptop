...
const {getCursorWindowPoint} = require('../../../common/lib/browserWindowUtil')
...
const DRAG_DETACH_PX_THRESHOLD_POSTSORT = 90
const DRAG_DETACH_MS_TIME_BUFFER = 300
...
class Tab extends React.Component {
  ...
  reportMoveToOtherWindow (mouseMoveEvent) {
    ...
    const {x: clientX, y: clientY} = getCursorWindowPoint(win, { x: mouseMoveEvent.screenX, y: mouseMoveEvent.screenY })
    ...
    if (!this.props.draggingDisplayIndexRequested || this.props.draggingDisplayIndexRequested === this.props.displayIndex) {
      ...
      if (isOutsideBounds) {
        ...
        this.draggingDetachTimeout = this.draggingDetachTimeout || window.setTimeout(() => {
          appActions.tabDragDetachRequested(this.parentClientRect.x, this.parentClientRect.y)
