  ...
  switch (action.get('actionType')) {
    ...
    case appConstants.APP_TAB_DRAG_CHANGE_DISPLAY_INDEX: {
      ...
      state = state.mergeIn([stateKey], {
        // UI will need to update the 'shift' of the tab, so let it know where the mouse is
        // since it will only get it on mouse move which may have already happened
    ...
    case appConstants.APP_WINDOW_READY: {
      ...
      if (dragSourceData.has('detachRequestedWindowId')) {
        ...
        if (windowId === dragSourceData.get('detachRequestedWindowId')) {
        ...
        // focus the window so it gets mousemove event
        setImmediate(() => {
          // move the window as the user may not have moved mouse yet
          const win = BrowserWindow.fromId(windowId)
          win.focus()
          // store the last position so we can calc delta next time
          const mouseScreenPos = screen.getCursorScreenPoint()
          const relativeTabX = dragSourceData.get('relativeXDragStart')
          const relativeClientY = dragSourceData.get('originClientY')
          const windowY = Math.floor(mouseScreenPos.y - relativeClientY)
          const windowX = Math.floor(mouseScreenPos.x - relativeTabX)
    ...
    case appConstants.APP_TAB_ATTACHED: {
      ...
      if (dragSourceData.has('attachRequestedWindowId')) {
        ...
        const cursorWindowPoint = browserWindowUtil.getCursorWindowPoint(win)
        state = state.mergeIn([stateKey], {
          currentWindowId,
    ...
    case appConstants.APP_TAB_DRAG_SINGLE_TAB_MOVED: {
      ...
      const intersectionWindow = otherWindows.find(win => {
        ...
        const winRight = winX + win.getSize()[1]
        const winTabY = winY + action.get('tabY')
        if (
          mouseScreenPos.x > winX &&
          mouseScreenPos.x < winRight &&
          mouseScreenPos.y < winTabY + (yThreshold / 2) &&
          mouseScreenPos.y > winTabY - (yThreshold / 2)
        ) {
          return true
        }
      ...
      if (intersectionWindow) {
        ...
        // tab 'looks' attached)
        const cursorWindowPoint = browserWindowUtil.getCursorWindowPoint(intersectionWindow, mouseScreenPos)
        state = state.mergeIn([stateKey], {
          dragWindowClientX: cursorWindowPoint.x,
          dragWindowClientY: cursorWindowPoint.y,
    ...
    case appConstants.APP_TAB_DRAG_DETACH_REQUESTED: {
      ...
      setImmediate(() => {
        ...
        const browserOpts = {
          ...
          // provide offset now rather than calc exact X and Y now
          // as the window does not get created for some time
          // so move the calc closer to that time
          offsetX: Math.floor(-relativeTabX - tabX - frameLeftWidth),
          offsetY: Math.floor(-relativeTabY - tabY - frameTopHeight)
        }
        const frameOpts = frameOptsFromFrame(dragSourceData.get('frame'))
